...
from modules.mixers.cqmix import CausalMixer
...
class CQMIX_SMAC_Learner:
    def __init__(self, mac, scheme, logger, args):
        ...
        args.n_variables = args.enemy_shape * args.n_enemies
        ...
        elif args.mixer == "cqmix":
            self.mixer = CausalMixer(args)
        else:
            raise "mixer error"
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        # with th.no_grad():
            ...
            if getattr(self.args, 'q_lambda', False):
                ...
                if self.args.mixer == "cqmix":
                    qvals = self.target_mixer(qvals, causal_relation, batch["state"])
                else:
        ...
        # Mixer
        if self.args.mixer == "cqmix":
            chosen_action_qvals = self.mixer(chosen_action_qvals, causal_relation, batch["state"][:, :-1])
        else:
    ...
    def writereward(self, path, reward, mask, td_loss, loss, OPP_eval_Loss, 
        ...
        if os.path.isfile(path):
            ...
            with open(path, 'w') as f:
                csv_write = csv.writer(f)
                csv_write.writerow(
                    ['step', 'reward', 'TD_Loss', 'Final Loss', 'ally_alive', 'enemy_alive', 't_win_rate',])
                csv_write.writerow(
                    [step, reward.item(), td_loss.item(), loss.item(), ally_alive.item(), enemy_alive.item(),
                     win_rate.item(),])
